...
pub fn merchant_crm_routes() -> Router<Arc<MerchantCrmService>> {
    Router::new()
        // Customer profiles
        .route(
            "/merchant/:merchant_id/crm/customers",
            post(opt_in_customer),
        )
        .route("/merchant/:merchant_id/crm/customers", get(list_customers))
        .route(
            "/merchant/:merchant_id/crm/customers/:wallet_address",
            get(get_customer),
        )
        .route(
            "/merchant/:merchant_id/crm/customers/:wallet_address/tags",
            patch(update_tags),
        )
        // Retention
        ...
        // Export
        .route(
            "/merchant/:merchant_id/crm/export/anonymised",
            get(export_anonymised),
        )
}
