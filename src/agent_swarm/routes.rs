...
pub fn agent_swarm_routes(state: SwarmState) -> Router {
    Router::new()
        ...
        .route(
            "/agent-swarm/tasks/:task_id/micro-tasks",
            get(list_micro_tasks),
        )
        .route(
            "/agent-swarm/micro-tasks/:micro_task_id/submit",
            post(submit_micro_task),
        )
        // Consensus / voting
        ...
        // Settlements
        .route(
            "/agent-swarm/tasks/:task_id/settlements",
            get(list_settlements),
        )
        .route(
            "/agent-swarm/settlements/:settlement_id/confirm",
            post(confirm_settlement),
        )
        .with_state(state)
}
