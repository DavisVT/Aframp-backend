...
impl MerchantMultisigService {
    ...
    fn assert_not_frozen_sync(is_frozen: bool) -> MultisigResult<()> {
        if is_frozen {
            Err(MultisigError::AccountFrozen)
        } else {
            Ok(())
        }
    }
    ...
    pub async fn create_policy(
        ...
        merchant_id: &str,
        creator_id: &str,
        req: CreateSigningPolicyRequest,
    ) -> MultisigResult<SigningPolicy> {
        let id = Uuid::new_v4();
        sqlx::query(
    ...
    async fn find_applicable_policy(
        ...
        Err(MultisigError::NoPolicyApplicable(
    ...
    pub async fn create_group(
        ...
        merchant_id: &str,
        creator_id: &str,
        req: CreateSigningGroupRequest,
    ) -> MultisigResult<SigningGroup> {
        let id = Uuid::new_v4();
        sqlx::query(
    ...
    ) -> MultisigResult<SigningGroupMember> {
    ...
    pub async fn create_proposal(
        ...
        let policy = self
            .find_applicable_policy(merchant_id, &req.action_type, amount_bd.as_ref())
            .await?;
    ...
    pub async fn execute_proposal(
        ...
        sqlx::query(
            "UPDATE merchant_proposals SET status='executed', executed_at=now() WHERE id=$1",
        )
        .bind(proposal_id)
        .execute(&self.pool)
        .await?;
...
impl From<FreezeStateRow> for FreezeState {
    fn from(r: FreezeStateRow) -> Self {
        Self {
            ...
            frozen_by_name: r.frozen_by_name,
            freeze_reason: r.freeze_reason,
            frozen_at: r.frozen_at,
            unfrozen_by: r.unfrozen_by,
            unfrozen_at: r.unfrozen_at,
            unfreeze_reason: r.unfreeze_reason,
...
impl From<SignatureRow> for ProposalSignature {
    fn from(r: SignatureRow) -> Self {
        Self {
            ...
            decision: if r.decision == "approved" {
                SignerDecision::Approved
            } else {
