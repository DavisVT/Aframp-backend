...
pub fn agent_dashboard_routes(state: DashboardState) -> Router {
    Router::new()
        ...
        .route(
            "/agent-dashboard/agents/:agent_id/intervene",
            post(intervene),
        )
        .route(
            "/agent-dashboard/agents/:agent_id/interventions",
            get(list_interventions),
        )
        // Human Approval Queue
        .route("/agent-dashboard/approvals", get(list_approvals))
        .route(
            "/agent-dashboard/approvals/:item_id/decide",
        ...
        // Audit export
        .route(
            "/agent-dashboard/agents/:agent_id/audit-export",
            get(audit_export),
        )
        .with_state(state)
}
