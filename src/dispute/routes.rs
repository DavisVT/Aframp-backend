...
pub fn dispute_routes() -> Router<Arc<DisputeService>> {
    Router::new()
        ...
        .route(
            "/disputes/:id/evidence/customer",
            post(submit_customer_evidence),
        )
        // Merchant — mediation dashboard
        .route(
            "/disputes/merchant/:merchant_id",
            get(list_merchant_disputes),
        )
        .route("/disputes/:id/respond", post(merchant_respond))
        .route(
            "/disputes/:id/evidence/merchant",
            post(submit_merchant_evidence),
        )
        // Platform mediation
